...
## Data
> I took the data from Table 3.1 (Mackinnon, 2008, p. 56) [[**.csv**](https://raw.githubusercontent.com/nmmichalak/nicholas_michalak/master/blog_entries/2018/nrg01/data/mackinnon_2008_t3.1.csv)]
...
> * `~` = Regress on
> * Within the regression models, I label coefficients with the star.
> * `:=` = define a new parameter. Note that the stars after `:=` mean multiply
...
## Interpretation
Every 1-degree (Fahrenheit) increase in room temperature was associated with an *a* = 0.339 (*S.E.* = 0.101) increase in thirstiness units. Adjusting for room temperature, every 1-unit increase in thirstiness was associated with drinking *b* = 0.451 (*S.E.* = 0.149) more deciliters of water. Increases in room temperature were associated with increases in water drinking indirectly through increases in thirstiness. Specifically, for every *a* = 0.339 unit increase in the association between room temperature and thirstiness, there was an *ab* = 0.153 (*S.E.* = 0.064) increase in deciliters of water people drank. Importatnly, a bias-corrected bootstrapped confidence interval with 10,000 samples was above zero, 95% CI [0.06, 0.32]. There was no sufficient evidence that room temperature was associated with how many deciliters of water people drank independent of its association with thirstiness, *c* = 0.208 (*S.E.* = 0.130).
...
> The plot above depicts the relationship between the proposed mediator (thirstiness) and outcome variable (water drank, in dl) at different levels of the proposed antecedent (room temperature, in degrees Fahrenheit). The plot doesn't include this, but if you draw a right triangle in between the vertical lines marking the *a* coefficient and using the slope as the hypotenuse, then you can eyeball the indirect effect, which is height of this triangle. I wish this function drew that triangle and labeled the indirect effect. Close enough, I guess.

## Test same model using `mediate()` from psych
