...
> It could be easier to interpret the covariance as an "average of the X-Y matches": X scores above the X mean multipled by Y scores below the Y mean will be negative, and X scores above the X mean multipled by Y scores above the Y mean will be positive. More "mismatches" leads to a negative covariance and more "matches" leads to a positive covariance.
...
> Below, I create the lines and the confidence "ribbons" from the tables I created above. The points come from the original `data.frame` though.
